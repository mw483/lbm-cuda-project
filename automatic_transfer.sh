PID=$(pgrep -u $(whoami) -f mpirun.sh) #PID of runlbm.sh, check the user also to make sure I am checking my simulation
SRC_CSV="./Output"
...
if [ -z "$PID" ]; then
    ...
    echo "Output CSV will be moved to $DEST_CSV and Particle Binary will be moved to $DEST_PAR"
